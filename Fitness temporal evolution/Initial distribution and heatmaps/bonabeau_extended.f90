...
double precision t1,t2,t3,tmax, told, tini, t, tn, dran_u !Times
...
open (30,file='posN.dat')
open (40,file='posN2.dat')

!Inicialize the same random string at each different density simulation.
call dran_ini(ILLAV)
...
!Storing positions and their fitness
do i=1,N
      write(30,*) i, pos(1,i), pos(2,i), fitness(i)
enddo

do j=1,N2
      write(40,*) j, pos2(1,j), pos2(2,j), fitness2(j)
enddo

write(*,*) "Begining of the MC loop", L
!Montecarlo loop:
!Time variables
tini=20000.d0 !First time
told=0.d0 ! To store the exact 20.000
tmax=600000.d0 !tmax
...
do while (t.lt.tmax) !bucle over Gillespie time
      ...
      ! writing positions at tini
      if ((t.ge.tini).and.(told.lt.tini)) then
            write(30,*)
            write(30,*)
            write(40,*)
            write(40,*)
            ...
            CALL CPU_TIME(t2)
            write(*,*) "Simulation time", t, "Partial CPU TIME", t2-t1
      endif
...
CALL CPU_TIME(t3)
write(*,*) "Simulation time", t, "Total CPU TIME", t3-t1
