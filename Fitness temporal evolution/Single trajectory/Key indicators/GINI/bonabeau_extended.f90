...
implicit none
integer i,j,i_dran,L,N,N2, k, h, nvec, n2vec
...
integer*2, ALLOCATABLE :: pos(:,:) !Positions in the square lattice
integer*2, ALLOCATABLE :: pos2(:,:)
double precision, ALLOCATABLE :: fitness(:) !Agent fitnesses
double precision, ALLOCATABLE :: fitness2(:)
double precision eta, fraction, omega, omega2, omegatot, totalfitness !Model Parameters
...
nvec=[1000,500,350,700]
n2vec=[100,50,200,400]
eta=5.0d0
...
CALL CPU_TIME(t1)
!Opening files to store the data
open (20,file='fitnessagents.dat')
open (70,file='histogramfitness.dat')
open (80,file='histogramfitness2.dat')
open (90,file='histogramtotal.dat')
...
N=nvec(h) !Number of agents A
N2=n2vec(h) !Number of agents B
...
!Inicialize the same random string at each different agents number simulation.
call dran_ini(ILLAV)
ALLOCATE(pos(1:2,1:N))
ALLOCATE(pos2(1:2,1:N2))
ALLOCATE(fitness(1:N))
ALLOCATE(fitness2(1:N2))
...
write(*,*) "Begining of the MC loop", N,N2
! Montecarlo loop:
!time variables
tmax=600000.d0 !tmax
...
write(20,*) t, maxval(fitness), maxval(fitness2), sum(fitness), sum(fitness2)
...
!Write to make a histogram of the agents A.
do i=1,N
      write(70,*) fitness(i)
      write(90,*) fitness(i)
enddo

!Write to make a histogram of the agents B.
do i=1,N2
      write(80,*) fitness2(i)
      write(90,*) fitness2(i)
enddo
...
CALL CPU_TIME(t2)
write(*,*) "Finish agents:", N,N2,  "Partial CPU TIME", t2-t1, sum(fitness)+sum(fitness2)
DEALLOCATE(pos)
DEALLOCATE(pos2)
DEALLOCATE(fitness)
DEALLOCATE(fitness2)
! End loop agents number
enddo
CALL CPU_TIME(t3)
