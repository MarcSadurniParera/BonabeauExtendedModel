...
integer i,j,i_dran,L,N,N2, k, h, run, n_run, Lmin, Lmax, Lstep, Linterval, survivors !Model Parameters
parameter(N=500,N2=50) !number of NA=N and NB=N2
parameter(n_run=100) !Number of Runs
...
double precision t1,t2,t3,MonopTime, t, tn, dran_u !Times
...
open (90,file='MonopTime.dat')

! Loop under different System sizes
do h=0,Lstep
...
do while (survivors.ne.1) !bucle over Gillespie time
      ! Gillespie Algorithm      
      ...
      ! Counting survivors.
      if(MAXVAL(fitness2).gt.175.d0) then
            survivors=1
      endif
...
MonopTime=MonopTime+t

enddo !end loop Runs

write(90,*) L, MonopTime/dble(n_run)
